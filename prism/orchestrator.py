"""
PRISMOrchestrator: top-level entry point that wires together the entire
two-pass pipeline.
...
Usage:
    from prism import PRISMOrchestrator, PRISMConfig, LogConfig
...
class PRISMOrchestrator:
    """
    End-to-end orchestrator for the PageRank-indexed, Symbol-aware Model.
    ...
    def run(
        ...
        """
        Execute the full PRISM pipeline for a given task.
    ...
    def run_coding_session(
        ...
        Args:
            ...
            task:    Natural language task description.
            resume:  If True, resume the existing plan from .prism_plan.json
        ...
        if resume:
            ...
            if plan is None:
                self._logger.warn(
                    ...
                    hint="No .prism_plan.json found — starting fresh session",
                )
