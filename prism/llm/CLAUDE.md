# prism/llm — LLM Abstraction Layer
...
- Provider is selected via `PRISMConfig.provider`.
...
```python
# Anthropic (default)
config = PRISMConfig(provider='anthropic', synthesis_model='claude-sonnet-4-6')

# Ollama (local)
config = PRISMConfig(provider='ollama', synthesis_model='llama3.2')
